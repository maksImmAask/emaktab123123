...
function Grades() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setSchedules(
        unwrapList<Schedule>(sch.data)
      );

      setData(
        unwrapList<Grade>(g.data)
      );
          ...
          >
            <Select
              options={schedules.map(
                (s) => ({
                  value: s.id,
                  label: `${s.school_class.name} • ${s.subject.name} • Lesson ${s.lesson_number}`,
                })
              )}
            />
          </Form.Item>

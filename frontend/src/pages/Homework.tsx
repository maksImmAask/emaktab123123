...
import type { Homework, Schedule } from "../types";
...
function HomeworkPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setData(unwrapList<Homework>(h.data));
      setSchedules(unwrapList<Schedule>(s.data));
    } catch {
          ...
          >
            <Select
              options={schedules.map((s) => ({
                value: s.id,
                label: `Class ${s.school_class.name} - ${s.subject.name}`,
              }))}
            />
          </Form.Item>
